...
export default class WheelPainter {
	...
	drawBackground(context: Context, wheel: Wheel) {
		...
		if (!this.imageCache.has('background')) {
			this.drawBackgroundNoCache(createInMemoryImage(context), wheel.config.image);
		}
	...
	drawSlice(context: Context, wheel: Wheel, index: number) {
		...
		if (wheel.config.type === 'color') {
			...
			this.drawText(context, wheel.entries[index].text, radius, getTextColor(bgColor));
		} else {
			this.drawText(context, wheel.entries[index].text, radius, 'white', 'black');
		}
	}

	drawSliceBg(context: Context, radius: number, radians: number, color: string) {
		context.beginPath();
		context.moveTo(0, 0);
	...
	drawText(context: Context, text: string, radius: number, color: string, strokeColor?: string) {
	...
	drawCenter(context: Context, wheel: Wheel) {
		...
		if (!this.imageCache.has('center')) {
			this.drawCenterNoCache(createInMemoryImage(context), hubSizes[wheel.config.hubSize]);
		}
	...
	async drawCenterImageNoCache(context: Context, dataUri: string, hubSize: number) {
...
const createInMemoryImage = (context: Context) =>
	new OffscreenCanvas(context.canvas.width, context.canvas.height).getContext('2d')!;
