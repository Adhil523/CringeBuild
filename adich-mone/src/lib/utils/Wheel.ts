...
import { initialWheelState, click, tick, type WheelState } from '$lib/utils/WheelState';
...
export default class Wheel {
	...
	tick() {
		const oldState = this.state;
		const newState = tick(oldState, this.config.spinTime, this.config.indefiniteSpin);
...
const getIndexAtPointer = (wheel: Pick<Wheel, 'entries' | 'state'>) => {
	return (
		Math.round(wheel.state.angle / ((2 * Math.PI) / (wheel.entries.length || 1))) %
...
const getColorAtPointer = (wheel: Pick<Wheel, 'config' | 'entries' | 'state'>) => {
...
export const defaultEntries: Entry[] = ['1', '2', '3', '4', '5', '6'].map((text) => ({
	text,
	id: getNewEntryId()
}));
