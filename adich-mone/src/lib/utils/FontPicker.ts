...
type Context = CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D | SKRSContext2D;
...
export default class FontPicker {
	...
	getFont(
		...
		const cachedResult = this.getCachedResult(texts, wheelRadius, hubRadius, smallestAngle);
		if (cachedResult) return cachedResult;
		let minFontSize = 200;
		const fontName = 'Quicksand';
		texts.forEach((text) => {
			const fontSize = getFontSize(context, text, fontName, wheelRadius, hubRadius, smallestAngle);
	...
	getCachedResult(texts: string[], wheelRadius: number, hubRadius: number, smallestAngle: number) {
		return this.resultCache.get(getCacheKey(texts, wheelRadius, hubRadius, smallestAngle));
	}
	...
	cacheResult(
		...
		this.resultCache.set(getCacheKey(texts, wheelRadius, hubRadius, smallestAngle), font);
	}
...
const boxFits = (
	...
	return (
		(radius ** 2 - (height / 2) ** 2) ** 0.5 -
			Math.max((height * Math.cos(angle)) / (2 * Math.sin(angle)), innerRadius) >=
		width
	);
};
