...
export const tick = (state: WheelState, spinTime: number, indefiniteSpin: boolean): WheelState => {
	const processTickForPhase: Record<Phase, WheelStateFn> = {
		demo: (state: WheelState) => ({ ...state, speed: DEMO_SPEED }),
		accelerating: (state: WheelState) => tickAcceleratingPhase(state, spinTime, indefiniteSpin),
	...
	return increaseTicksInPhase(processTickForPhase[state.phase](increaseAngle(state)));
};
...
const tickDeceleratingPhase = (state: WheelState, spinTime: number): WheelState => {
...
const getDecelRate = (spinTime: number) => {
	return Math.exp(
		Math.log(STOP_SPEED / (DEMO_SPEED + getAccelTicks(spinTime) * getAccelRate())) /
